...
#define GEN_CAMERA_PHOTO_FILENAME(OUT_FILENAME_STR, OUT_THUMBNAIL_FILENAME_STR, DIR, THUMBNAIL_DIR, FORMAT) \
    struct timeval curTime; \
    gettimeofday(&curTime,0); \
    time_t timep; \
    struct tm *p; \
    ...
    char pathThumbnailPhotoFileName[1024]; \
    sprintf(pathThumbnailPhotoFileName, "%s/%s", THUMBNAIL_DIR, photoFileName); \
    (OUT_THUMBNAIL_FILENAME_STR) = std::string(pathThumbnailPhotoFileName);

#define GEN_CAMERA_MP4_FILENAME(OUT_FILENAME_STR, OUT_THUMBNAIL_FILENAME_STR, DIR, THUMBNAIL_DIR, FORMAT) \
    struct timeval curTime; \
    gettimeofday(&curTime,0); \
    time_t timep; \
    struct tm *p; \
    ...
    char pathThumbnailMediaFileName[1024]; \
    sprintf(pathThumbnailMediaFileName, "%s/%s.jpg", THUMBNAIL_DIR, mediaFileName); \
    (OUT_THUMBNAIL_FILENAME_STR) = std::string(pathThumbnailMediaFileName);